#Autors: Manuel Mollón, Tomás Gonzalez Orlando, Matías Larroque.
#program mainEj4.py: In order to show the effect of the contrast in a picture, this code shows two pictures
# each one presents two rectangles: one central rectangle and one background rectangle. The shade of gray of the central rectangle
# is the same in each image, but the background change, so the contrast effect is shown because the central rectangles seem to be differents.
...
while (not(windowsAreClosed)):
    ...
    windowsAreClosed = not(bool(cv.getWindowProperty('Imagen1',cv.WND_PROP_VISIBLE)) or bool(cv.getWindowProperty('TrackBars',cv.WND_PROP_VISIBLE)))